...
function OrgSettingsSection({ org, isAdmin, onChange }: { org: OrganizationSettings; isAdmin: boolean; onChange: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-sm font-medium text-slate-200">Organization Settings</h2>
      ...
      {isAdmin ? (
        <>
          <div className="flex gap-2 max-w-md">
            <input
              ...
              className="flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-sm text-slate-200"
            />
            <button
          ...
          <div className="flex items-center gap-3">
            <p className="text-sm text-slate-400">
              Invite code: <span className="font-mono text-slate-200">{org.slug}</span>
            </p>
            <button
              onClick={() => void rotateCode()}
              disabled={busy}
              className="rounded-lg border border-amber-800 hover:bg-amber-900/40 disabled:opacity-50 text-xs px-2.5 py-1 text-amber-300 transition"
            >
              Rotate Invite Code
            </button>
          </div>
        ...
        <p className="text-sm text-slate-400">
          {org.name} &middot; invite code <span className="font-mono text-slate-200">{org.slug}</span>
        </p>
      )}
    </div>
  )
...
function ApiKeysSection() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-slate-500">For programmatic access (CI, external integrations) - alternative to a JWT login.</p>

      {error && <p className="text-sm text-red-400">{error}</p>}

      {justCreated && (
        <div className="rounded-lg border border-emerald-800 bg-emerald-950/30 p-3 space-y-1">
          <p className="text-xs text-emerald-300">
            Copy this key now - it will not be shown again: <button onClick={() => setJustCreated(null)} className="float-right text-slate-500 hover:text-slate-300">dismiss</button>
          </p>
          <code className="block text-sm text-slate-100 break-all">{justCreated.key}</code>
        </div>
      )}
      ...
      <div className="divide-y divide-slate-800">
          ...
          <div key={key.id} className="flex items-center justify-between py-2">
            <div>
              <p className="text-sm text-slate-100">
                {key.name} <span className="font-mono text-xs text-slate-500">{key.key_prefix}...</span>
              </p>
        ...
        {keys.length === 0 && <p className="text-sm text-slate-500 py-2">No API keys yet.</p>}
      </div>
    </div>
  )
}
      ...
      <div className="divide-y divide-slate-800">
        {entries.map((entry) => (
          <div key={entry.id} className="py-2 text-sm">
            <span className="text-slate-200">{entry.action.replace(/_/g, ' ')}</span>
            <span className="text-slate-500"> by {entry.actor_email} &middot; {entry.created_at}</span>
            <pre className="text-xs text-slate-500 mt-1 whitespace-pre-wrap">{JSON.stringify(entry.details)}</pre>
          </div>
        ))}
        {entries.length === 0 && <p className="text-sm text-slate-500 py-2">No audit entries yet.</p>}
      </div>
    </div>
  )
}
...
export function EnterpriseAdminPage() {
  ...
  if (loading || !org) {
    return <div className="text-slate-400">Loading enterprise admin...</div>
  }

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-lg font-semibold text-slate-100">Enterprise Administration</h1>
        <p className="text-sm text-slate-500 mt-1">Organization settings, API keys, and the security audit log.</p>
      </div>
      ...
      {isAdmin ? (
        ...
        <p className="text-sm text-slate-500">API keys and the audit log are only visible to organization admins.</p>
      )}
    </div>
  )
}
