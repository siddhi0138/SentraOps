...
export function NotificationBell() {
  ...
  return (
    <div ref={containerRef} className="relative">
      <button onClick={() => setOpen((v) => !v)} className="relative text-slate-400 hover:text-white transition p-1">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 max-h-96 overflow-y-auto rounded-lg border border-slate-700 bg-slate-900 shadow-xl z-50">
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((n) => (
              <button
                ...
                className={`block w-full text-left px-3 py-2 text-sm border-b border-slate-800 last:border-0 hover:bg-slate-800 transition ${
