...
Both are optional — the app works fine without either. Slack gets you incident
alerts and Approve/Reject buttons in a channel; Jira gets you an automatic
ticket for every approved response action. Full step-by-step setup (Slack
app manifest, ngrok tunneling for local dev, Jira API tokens, the optional
two-way sync) lives in **[INTEGRATIONS.md](INTEGRATIONS.md)** — kept
separate so this README stays scannable.
